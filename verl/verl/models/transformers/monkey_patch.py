...
def _ulysses_flash_attention_forward(
    ...
    **kwargs,
    ...
    if ulysses_sp_size > 1 and position_ids is not None:
        ...
        # NOTE(qwen3.5-sp-fix from nev8rz/verl@9625d385): position_ids may be a sliced
        # non-contiguous view; all_gather requires contiguous input.
...
def apply_monkey_patch(
    ...
    if model.config.model_type in ["qwen2_5_vl", "qwen2_vl"]:
        ...
        # Step 3: patch input for multimodal sequence parallelism
        # NOTE(qwen3.5-sp-fix from nev8rz/verl@9625d385): without this the input is
        # not sliced across SP ranks, causing the temperature/logits 28310-vs-14155
        # tensor mismatch reported in verl-project/verl#5957 and #6094.
        if ulysses_sp_size > 1:
